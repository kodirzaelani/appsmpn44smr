                ...
                <div class="box box-bordered border-primary">
                    <div class="box-header with-border">
                        ...
                        <div class="box-controls pull-right">
                            <a class="btn btn-xs btn-primary" href="{{ route('backend.sliders.create') }}"><i
                            ...
                            <div class="col-xl-12 col-md-12 col-lg-12 col-12">
                                @if ($selectPage)
                                    <div class="col-md-10 mb-2">
                                        @if ($selectAll)
                                            <div>
                                                You have selected all <strong>{{ $datasliderall->total() }}</strong>
                                                items.
                                            </div>
                                        @else
                                            <div>
                                                ...
                                                want to Select All
                                                <strong>{{ $datasliderall->total() }}</strong>?
                                ...
                                @if ($datasliderall->count())
                                    <div class="table-responsive">
                                        <table class="table table-hover mb-0">
                                            ...
                                            <tbody>
                                                @foreach ($datasliderall as $no => $item)
                                                    <tr class="@if ($this->isChecked($item->id)) table-primary @endif">
                                                        <th class="text-right" scope="row">
                                                            {{ $no + $datasliderall->firstItem() }}</th>
                                                        {{-- <td>
                                                    <div class="checkbox">
                                                        <input type="checkbox" id="Checkbox_{{ $no + $datasliderall->firstItem() }}" value="{{ $item->id }}" wire:model="checked">
                                                        <label for="Checkbox_{{ $no + $datasliderall->firstItem() }}"></label>
                                                    </div>
                                                </td> --}}
                                                        <td width="10%">
                                                        ...
                                                        <td class="text-center align-midle">
                                                            ...
                                                            @if ($item->image)
                                                                <button
                                                                    wire:click="selectItem('{{ $item->id }}', 'imageremove')"
                                                                    class="btn btn-xs btn-danger"
                                                                    title="Remove Image"><i
                                                                        class="fa fa-image"></i></button>
                                                            @else
                                                                <button class="btn btn-xs btn-default disable"
                                                                    title="No Image"><i
                                                                        class="fa fa-image"></i></button>
                                                            @endif
                                                            <a title="Edit" class="btn btn-xs btn-warning edit-row"
                                                                href="{{ route('backend.sliders.edit', $item->id) }}">
                                    ...
                                    <div class="mt-3 row">
                                        ...
                                        <div class="col-xl-5 col-md-5 col-lg-5 col-12 text-center">
                                            Page : {{ $datasliderall->currentPage() }} | Show
                                            {{ $datasliderall->count() }} data of {{ $datasliderall->total() }}
                                        </div>
                                    </div>
                                @else
                                    <hr>
