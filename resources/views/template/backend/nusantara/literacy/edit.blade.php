...
@section('content')
                    ...
                    <nav>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item" aria-current="page"><a
                                    href="{{ route('backend.literacies.index') }}">All Literacy</a></li>
    ...
    <section class="content">
        <form id="post-form" enctype="multipart/form-data" action="{{ route('backend.literacies.update', $literacy) }}"
            method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                ...
                                <div class="controls">
                                    <input type="text" name="title"
                                        ...
                                        value="{{ old('title') ?? $literacy->title }}" placeholder="Title" required>
                                </div>
                                @error('title')
                            ...
                            <div class="form-group">
                                ...
                                <textarea rows="5" cols="80" class="form-control @error('urlcontent') is-invalid @enderror"
                                    name="urlcontent">{{ old('urlcontent') ?? $literacy->urlcontent }}</textarea>
                                <small>Example :
                                    file/d/0B8TiTqB1Sz_5ZUFZOFFCWGdUWk0/preview?resourcekey=0-tLJ0mPL_xz6diUFSzGf7Vw</small>
                                @error('urlcontent')
                                    <div class="form-control-feedback"><small> <code>{{ $message }}</code> </small></div>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <div class="controls">
                                    <input type="text" name="linkdownload"
                                        class="form-control @error('linkdownload') is-invalid @enderror"
                                        value="{{ old('linkdownload') ?? $literacy->linkdownload }}" placeholder="ID File">
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="editor1" rows="10" cols="80" class="form-control @error('content') is-invalid @enderror"
                                    name="content">{{ old('content') ?? $literacy->content }}</textarea>
                                @error('content')
                                    <div class="form-control-feedback"><small> <code>{{ $message }}</code> </small></div>
                                @enderror
                            </div>
                                ...
                                @if ($literacy->urlcontent)
                                    <div class="box border-0 rounded-lg shadow-sm">
                                        <div class="text-center box-body">
                                            <iframe src="https://drive.google.com/{{ $literacy->urlcontent }}"
                                                width="540" height="480" allow="autoplay"></iframe>
                                        </div>
                                    </div>
                                @endif
                            </div>
                    ...
                    <div class="box">
                                ...
                                <select class="form-control select2" style="width: 100%;" name="literacycategory_id">
                                    ...
                                    @foreach ($literacycategories as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('literacycategory_id') == $item->id ? 'selected' : '' }}
                                            @if ($item->id == $literacy->literacycategory_id) selected @endif>
                                            {{ $item->title }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="box-body">
                            <div class="form-group @error('literacysubcategory_id') has-error @enderror">
                                ...
                                <select class="form-control select2" style="width: 100%;" name="literacysubcategory_id"
                                    id="literacysubcategory_id" disabled>
                                    <option value="" holder>Select Literacy Category</option>
                                    @foreach ($literacysubcategories as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('literacysubcategory_id') == $item->id ? 'selected' : '' }}
                                            @if ($item->id == $literacy->literacysubcategory_id) selected @endif>{{ $item->title }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="box-body  ">
                            <div class="form-group text-center">
                                <div class=" fileinput fileinput-new" data-provides="fileinput">
                                    <div class="fileinput-new img-thumbnail" style="width: 200px;">
                                        <img src="{{ $literacy->imageThumbUrl ? $literacy->imageThumbUrl : '/assets/images/no_image.png' }}"
                                            alt="...">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="caption_image" id="caption_image" class="form-control @error('caption_image') is-invalid @enderror"
                                    placeholder="Caption Image">{{ old('caption_image') ?? $literacy->caption_image }}</textarea>
                                @error('caption_image')
            ...
            $(document).ready(function() {
                $('select[name="literacycategory_id"]').on('change', function() {
                    ...
                    if (literacycategory_id) {
                        $.ajax({
                            url: "{{ url('/backend/get/literacysubcategory/') }}/" +
                                literacycategory_id,
                            type: "GET",
                            dataType: "json",
                            success: function(data) {
                                ...
                                $.each(data, function(key, value) {
                                    $("#literacysubcategory_id").append('<option value="' +
