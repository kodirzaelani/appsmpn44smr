...
@section('content')
    ...
    <section class="content">
        <form id="post-form" enctype="multipart/form-data" action="{{ route('backend.pages.store') }}" method="post">
            <div class="row">
                @csrf
                <div class="col-lg-8 col-12">
                    <div class="box">
                        <div class="box-header">
                            <h4 class="box-title">Create a Page
                                ...
                                <select class="form-control select2" style="width: 100%;" name="pagecategory_id">
                                    ...
                                    @foreach ($pagecatagories as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('pagecategory_id') == $item->id ? 'selected' : '' }}>{{ $item->title }}
                                        </option>
                                    @endforeach
                                </select>
