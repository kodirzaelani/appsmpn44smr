...
@section('content')
                ...
                <div class="d-inline-block align-items-center">
                    <nav>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item" aria-current="page"><a href="{{ route('backend.fotos.index') }}">All
                                    Foto</a></li>
    ...
    <section class="content">
        <form id="post-form" enctype="multipart/form-data" action="{{ route('backend.fotos.update', $foto) }}"
            method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                ...
                                <div class="controls">
                                    <input type="text" name="title"
                                        ...
                                        value="{{ old('title') ?? $foto->title }}" placeholder="Title" required>
                                </div>
                                @error('title')
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="editor1" rows="10" cols="80" class="form-control @error('excerpt') is-invalid @enderror"
                                    name="excerpt">{{ old('excerpt') ?? $foto->excerpt }}</textarea>
                                @error('excerpt')
                                    <div class="form-control-feedback"><small> <code>{{ $message }}</code> </small></div>
                                @enderror
                            </div>
                                ...
                                <select class="form-control select2" style="width: 100%;" name="album_id">
                                    ...
                                    @foreach ($albums as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('album_id') == $item->id ? 'selected' : '' }}
                                            @if ($item->id == $foto->album_id) selected @endif>
                                            {{ $item->title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                <div class=" fileinput fileinput-new" data-provides="fileinput">
                                    <div class="fileinput-new img-thumbnail" style="width: 200px;">
                                        <img src="{{ $foto->imageThumbUrl ? $foto->imageThumbUrl : '/assets/images/no_image.png' }}"
                                            alt="...">
